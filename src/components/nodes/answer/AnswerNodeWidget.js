...
export class AnswerNodeWidget extends RJD.DefaultNodeWidget {
  static defaultProps = {
    node: null,
  ...
  getInPort() {
    ...
    if (displayOnly) {
      inputNode = new AnswerNodeModel(node.name, color);
    }
  ...
  getOutPort() {
    ...
    if (displayOnly) {
      outputNode = new AnswerNodeModel(node.name, color);
    }
...
export const AnswerNodeWidgetFactory = React.createFactory(AnswerNodeWidget);
