...
export class BridgeNodeWidget extends RJD.DefaultNodeWidget {
  static defaultProps = {
    node: null,
    color: 'rgb(73, 72, 79)'
  };
  ...
  getInPort() {
    ...
    if (displayOnly) {
      inputNode = new BridgeNodeModel(node.name, color);
    }
  ...
  getOutPort() {
    ...
    if (displayOnly) {
      outputNode = new BridgeNodeModel(node.name, color);
    }
...
export const BridgeNodeWidgetFactory = React.createFactory(BridgeNodeWidget);
