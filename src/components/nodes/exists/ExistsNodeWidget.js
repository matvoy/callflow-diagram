...
export class ExistsNodeWidget extends RJD.DefaultNodeWidget {
  static defaultProps = {
    node: null,
  ...
  getInPort() {
    ...
    if (displayOnly) {
      inputNode = new ExistsNodeModel(node.name, color);
    }
  ...
  getOutPort() {
    ...
    if (displayOnly) {
      outputNode = new ExistsNodeModel(node.name, color);
    }
...
export const ExistsNodeWidgetFactory = React.createFactory(ExistsNodeWidget);
