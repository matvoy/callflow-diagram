...
export class LogNodeWidget extends React.Component {
  ...
  getInPort() {
    ...
    if (displayOnly) {
      inputNode = new LogNodeModel(node.name, color);
    }
  ...
  getOutPort() {
    ...
    if (displayOnly) {
      outputNode = new LogNodeModel(node.name, color);
    }
...
export const LogNodeWidgetFactory = React.createFactory(LogNodeWidget);
