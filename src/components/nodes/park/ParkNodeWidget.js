...
export class ParkNodeWidget extends React.Component {
  ...
  getInPort() {
    ...
    if (displayOnly) {
      inputNode = new ParkNodeModel(node.name, color);
    }
  ...
  getOutPort() {
    ...
    if (displayOnly) {
      outputNode = new ParkNodeModel(node.name, color);
    }
...
export const ParkNodeWidgetFactory = React.createFactory(ParkNodeWidget);
