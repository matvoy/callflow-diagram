...
export class PickupNodeWidget extends React.Component {
  ...
  getInPort() {
    ...
    if (displayOnly) {
      inputNode = new PickupNodeModel(node.name, color);
    }
  ...
  getOutPort() {
    ...
    if (displayOnly) {
      outputNode = new PickupNodeModel(node.name, color);
    }
...
export const PickupNodeWidgetFactory = React.createFactory(PickupNodeWidget);
