...
export class PlaybackNodeWidget extends RJD.DefaultNodeWidget {
  static defaultProps = {
    node: null,
    color: 'rgb(73, 72, 79)'
  };
  ...
  getInPort() {
    ...
    if (displayOnly) {
      inputNode = new PlaybackNodeModel(node.name, color);
    }
  ...
  getOutPort() {
    ...
    if (displayOnly) {
      outputNode = new PlaybackNodeModel(node.name, color);
    }
...
export const PlaybackNodeWidgetFactory = React.createFactory(PlaybackNodeWidget);
